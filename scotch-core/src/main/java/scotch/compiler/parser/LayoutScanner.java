...
import static scotch.compiler.parser.LayoutScanner.State.ACCEPT;
import static scotch.compiler.parser.LayoutScanner.State.SCAN_DEFAULT;
import static scotch.compiler.parser.LayoutScanner.State.SCAN_DISABLED;
import static scotch.compiler.parser.LayoutScanner.State.SCAN_LAYOUT;
import static scotch.compiler.parser.LayoutScanner.State.SCAN_LET;
...
public final class LayoutScanner implements Scanner {
    ...
    private void buffer() {
        if (tokens.isEmpty() || !lastToken().is(TokenKind.EOF) && tokens.size() < LOOK_AHEAD) {
            buffer_();
        }
    }
    ...
    private void layout_() {
        ...
        switch (currentState()) {
            ...
            case SCAN_LET:
                ...
                break;
            case SCAN_DISABLED:
                if (firstToken().is(LCURLY) || firstToken().is(LSQUARE)) {
                    bracesUp();
                } else if (firstToken().is(RCURLY) || secondToken().is(RSQUARE)) {
                    if (hasBraces()) {
                        bracesDown();
                    } else {
                        leaveState();
